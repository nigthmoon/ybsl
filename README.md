...
### 核心文件
- **扩展配置** (`config.js`): 扩展配置项、功能开关、帮助文档、QQ群二维码
- **包管理** (`package.js`): 包信息和版本管理
- **内容加载** (`content.js`): 游戏内容加载，调用所有content模块
- **预加载** (`precontent.js`): 预加载内容，导入所有precontent模块和核心函数
- **函数库** (`function.js`): 自定义通用函数库(50KB)，包含对象合并、武将介绍处理等
...
│   │   ├── YB_10_destiny.js     # 命运线系统 ❌ 代码被注释未启用
...
│
├── yblc/                        # 夜白旅程(闯关模式)
│   ├── YBLC_1.js                # 旅程关卡1(夜白旅程) ✅
│   ├── YBLC_2.js                # 旅程关卡2(寻你千百度) ✅
│   └── 遗憾可书.emmx           # 情感记录文件(思维导图)
│
├── sgstrxs/                     # 三国杀同人小说模式
│   └── mode.js                   # ❌ 文件不存在(目录sgstrxs不存在)
│
├── image/                       # 图片资源
│   ├── character/               # 武将头像
│   ├── card/                    # 卡牌图片
...
- YB_10_destiny.js (命运线系统) 代码被注释,未在precontent.js中导入,暂未启用
