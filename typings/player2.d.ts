/**
 * 夜白神略 - Player 对象扩展类型定义
 */

declare module "noname" {
	interface Player {
		/**
		 * 创建一个分配卡牌的事件
		 * - 遍历输入的元素，根据类型自动识别并赋值给 `cards`、`str` 和 `num`
		 * - 将 `cards` 中的 `num` 张牌分配给场上角色，分配时的提示信息为 `str`
		 *
		 * @param args 输入的元素（可以是卡牌组、字符串、数字、函数，顺序不固定）
		 * @returns 返回创建的事件对象
		 */
		YB_yiji(...args: (number | string | Card[] | ((card: Card, player: Player, target: Player) => boolean))[]): GameEvent;

		/**
		 * 创建一个分配卡牌的事件（粮营版）
		 * - 遍历输入的元素，根据类型自动识别并赋值给 `cards`、`boolyb`、`str` 和 `num`
		 * - 将 `cards` 中的 `num` 张牌分配给场上角色，分配时的提示信息为 `str`
		 * - `boolyb` 是一个布尔值，若为 true，则仅从输入卡牌中分配；若为 false，则从玩家所有手牌中选择
		 *
		 * @param args 输入的元素（可以是卡牌组、布尔值、字符串、数字、函数，顺序不固定）
		 * @returns 返回创建的事件对象
		 */
		YB_liangying(...args: any[]): GameEvent;

		/**
		 * 创建一个五谷丰登类型的事件
		 * @param cards 卡牌数组
		 * @param str 提示信息
		 * @param targets 目标角色
		 * @returns 返回创建的事件对象
		 */
		YB_wugu(cards: Card[], str: string, targets: Player[]): GameEvent;

		/**
		 * 获取角色的宗族
		 * @param unseen 是否包括暗置
		 * @returns 宗族数组
		 */
		getClan(unseen?: boolean): string[];

		/**
		 * 重置势极技状态
		 * @param i true 表示重置阴极，false 表示重置阳极
		 */
		YB_shiji(i?: boolean): void;

		/**
		 * 临时获得标记用的子技能并获得标记
		 * - 若没有对应的子技能会当场创建该子技能
		 *
		 * @param skill 此参数输入技能 id
		 * @param num 此参数输入获得的标记数
		 * @returns 事件对象
		 */
		YB_temp(skill: string, num?: number): GameEvent;

		/**
		 * 临时获得标记用的子技能并静默获得标记
		 * - 若没有对应的子技能会当场创建该子技能
		 *
		 * @param skill 此参数输入技能 id
		 * @param num 此参数输入获得的标记数
		 * @returns 事件对象
		 */
		YB_tempx(skill: string, num?: number): GameEvent;

		/**
		 * 临时获得标记用的子技能并显示该技能标记
		 * - 若没有对应的子技能会当场创建该子技能
		 *
		 * @param skill 此参数输入技能 id
		 * @param num 此参数输入获得的标记数
		 * @returns 事件对象
		 */
		YB_tempy(skill: string, num?: number): GameEvent;

		/**
		 * 临时获得子技能并将此 keys 添加进 skill 标记
		 * - 若没有对应的子技能会当场创建该子技能
		 *
		 * @param skill 此参数输入技能 id
		 * @param keys 要添加进 `storage[skill]` 的元素
		 * @returns 事件对象
		 */
		YB_tempz(skill: string, keys: any[]): GameEvent;

		/**
		 * 展示牌堆顶 `num` 张牌并获得其中每种花色各一张牌
		 * - 玩家选择时，不能选择多张同花色的牌
		 *
		 * @param num 展示的牌数
		 * @param i 提示信息
		 * @param log 是否强制选择最大数量
		 * @returns 事件对象
		 */
		YB_shelie(num: number, i?: string | true, log?: boolean): GameEvent;

		/**
		 * 扶汉函数 - 随机抽取武将并获得技能
		 *
		 * @param i 参数数组，包含势力、武将数、技能数、排除列表、性别、范围等
		 * @param type 特殊类型标记
		 * @returns 事件对象
		 */
		YB_fuhan(i: [string | string[], number?, number | string?, string[], Sex[] | "all", "zhu" | "nozhu"?], type?: "tw" | "old"): GameEvent;

		/**
		 * 将体力值调整至指定数值
		 *
		 * @param num 目标体力值
		 * @returns 事件对象
		 */
		YB_HpTo(num: number): GameEvent;

		/**
		 * 控制按钮翻页选择
		 *
		 * @param control 选项数组
		 * @param num 每页显示数量
		 * @param str 提示信息
		 * @returns 事件对象
		 */
		YB_control(control: string[], num?: number | string, str?: string): GameEvent;

		/**
		 * 隅泣函数，分配牌堆顶牌给不同角色
		 *
		 * @param i 数组，包含文字提示、展示数量、对方数量、自己数量等信息
		 * @param target 目标角色，不填为自己
		 * @returns 事件对象
		 */
		YB_yuqi(i: [string, number, number, number], target?: Player): GameEvent;

		/**
		 * 24 点算演小游戏
		 *
		 * @param cards 卡牌数组
		 * @param log 提示信息（但是好像没用到）
		 * @returns 事件对象
		 */
		FY_24(cards: Card[], log?: string): GameEvent;

		/**
		 * 文本输入选择
		 *
		 * @param args 多个参数，包括提示信息、过滤函数等
		 * @returns 事件对象
		 */
		FY_chooseText(...args: any[]): GameEvent;

		/**
		 * 升级指定技能
		 *
		 * @param str 要升级的技能 id 数组
		 */
		YB_levelUp(str: string[]): void;

		/**
		 * 翻牌小游戏
		 *
		 * @returns 事件对象
		 */
		YB_playTurnCard(): GameEvent;

		/**
		 * 命名小游戏
		 *
		 * @returns 事件对象
		 */
		YB_name(): GameEvent;

		/**
		 * 中流 - 重置所有技能的使用限制
		 */
		YB_zhongliu(): void;

		/**
		 * 重置指定技能
		 *
		 * @param skills 技能 id 或技能数组
		 */
		YB_zhongliuSkills(skills: string | string[]): void;

		/**
		 * 判断目标角色是否可以吃零食
		 *
		 * @returns 返回是否可以吃零食
		 */
		YB_canEat(): boolean;

		/**
		 * 判断目标角色饱食度上限
		 *
		 * @returns 返回饱食度上限
		 */
		YB_maxBaoshi(): number;

		/**
		 * 增加饱食度的函数，没有判断上限
		 *
		 * @param num 要增加的饱食度数值
		 */
		YB_addBaoshidu(num: number): void;

		/**
		 * 获取角色的最大蓝量值
		 *
		 * @returns 返回最大蓝量值
		 */
		YB_yzdel_maxMana(): number;

		/**
		 * 获取角色剩余可获得的蓝量值
		 *
		 * @returns 返回剩余蓝量空间
		 */
		YB_yzdel_noneMana(): number;

		/**
		 * 增加角色的蓝量值
		 *
		 * @param num 要增加的蓝量数值
		 */
		YB_gainMana(num: number): void;

		/**
		 * 减少角色的蓝量值
		 *
		 * @param num 要减少的蓝量数值
		 */
		YB_loseMana(num: number): void;

		/**
		 * 更新角色的 Mana 条显示
		 */
		YB_updateMana(): void;

		/**
		 * 久岛鸥的扬帆技能处理函数
		 *
		 * @param cards 要处理的手牌数组（可选，默认为所有手牌）
		 */
		kamome_ybyangfan(cards?: Card[]): void;

		/**
		 * 夜白神庞统相关函数 - 吸收卡组火焰
		 *
		 * @param list 卡牌数组
		 */
		YB_nofire(list: Card[]): void;

		/**
		 * 夜白神庞统相关函数 - 熄灭卡组火焰
		 *
		 * @param list 卡牌数组
		 */
		YB_fire(list: Card[]): void;

		/**
		 * 谋离间 - 令这些角色互相使用杀
		 *
		 * @param list 要进行操作的角色们
		 */
		YB_sblijian(list: Player[]): GameEvent;

		/**
		 * 将手牌数摸或弃至指定值
		 *
		 * @param num 要调整的目标数量
		 * @returns 事件对象
		 */
		YB_changeHandCard(num: number): GameEvent;

		/**
		 * 谋离间函数 - 令目标角色的邻居对其使用杀
		 */
		YB_sblijian(list: Player[]): GameEvent;
	}
}
