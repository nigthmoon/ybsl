import { Player } from "@types/noname-typings/nonameModules/noname/library/element/player";

declare module "@types/noname-typings/nonameModules/noname/library/element/player" {
	interface Player {
		/**
		 * 创建一个分配卡牌的事件。
		 ...
		 * @returns
		 */
		YB_fuhan(i: [group: string | string[], characterNum?: number, selectLimitOrId?: number | string, exclude?: string[], sex?: Sex[] | "all", scope?: "zhu" | "nozhu"], type?: "tw" | "old"): GameEvent;
		/**
		 * 令这些角色谋离间
		 ...
		 * @returns
		 */
		YB_yuqi(list: [tip: string, cardCount: number, targetLimit: number, playerLimit: number], target?: Player): GameEvent;
		/**
		 * 24点算演小游戏
		 * @param cards 卡牌数组
