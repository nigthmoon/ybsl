...
declare module "@/noname.js" {
  interface Get {
    ...
    YB_suit<
      T extends {
        [U in keyof Get]: U extends "YB_suit" | "event"
          ? never
          : unknown extends Parameters<Get[U]>[0]
            ? never
            : Parameters<Get[U]>[0] extends undefined
              ? never
              : Get[U] extends (card: Card) => unknown
                ? U
                : never;
      }[keyof Get],
    >(
     ...
     */
    characterLightext(
      list: [string, { YB_filterOk: (player: Player) => boolean }][],
      player?: Player,
    ): string[];
