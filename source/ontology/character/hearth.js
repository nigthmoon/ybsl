...
game.import("character", function (lib, game, ui, get, ai, _status) {
	var hearth = {
		...
		skill: {
			...
			bingyan: {
				...
				usable: 1,
				filter(event, player) {
					if (!lib.card.chiyuxi || !lib.card.jingleishan) {
						return false;
					}
					return player.countCards("he") > 0;
				},
	...
	get.typeimage(hearth,'ontology/character')
	for(var i in hearth.card){
		if(!hearth.card[i].image) hearth.card[i].image='extension/夜白神略/image/ontology/card/'+i+'.jpg'
	}
	return hearth;
});
