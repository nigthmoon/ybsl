...
game.import("card", function (lib, game, ui, get, ai, _status) {
		...
		skill: {
			...
			_gainspell: {
				...
				filter(event, player) {
					...
					if (!lib.config.cards.includes("gwent")) {
						return false;
					}
					if (player.isMin()) {
						return false;
	...
	for(var i in gwent.card){
		if (!gwent.card[i].cardimage) {
			if (gwent.card[i].fullskin) {
				gwent.card[i].image = "extension/夜白神略/image/ontology/card/" + i + ".png";
			} else {
				gwent.card[i].image = "extension/夜白神略/image/ontology/card/'+i+'.jpg";
			}
		}
