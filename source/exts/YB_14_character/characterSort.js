import { lib, game, ui, get, ai, _status } from '../../../../../noname.js'
export { characterSort }

const characterSort = {
	ybnew3:{
		ybsl_yjcm: [
			'ybslshen_pangtong'
		],
		ybsl_ybzs: [
			'ybsl_pujing','ybsl_zhouyuxiaoqiao',
		],
		ybsl_qygc: [
			'ybsl_xuyou','niya_re_liuyan','niya_caowei','Fe2O3_duqiong',
			'zxunnamed_zhenji','Fe2O3_zixu','Fe3O4_guanyu','Fe2O3_zhugeliang',
			'Fe2O3_zhaoyun', 'ddddssssbbbb_dengai', 'zxunnamed_gongsunyuan',
			'FeO3_zhugeliang', 'Fe3O4_zhugeliang'
		],
		ybsl_bqbs: [
			'ybsl_lvyi', 'ybsl_yinfan', 'ybsl_shlizhaoyi','ybsl_sunsháo',
			'ybsl_wangbi','ybnb_wangbi','ybsl_jiangziwen',
			'ybsl_suojing','ybsl_wangpou','ybsl_yangxu','ybsl_shichong',
			'ybsl_simayou',
		],
		ybsl_clanwjls: [
			'ybslclan_luxun', 'ybslclan_luyan', 'ybslclan_luji', 'ybslclan_lukang', 'ybslclan_lukai',
			'ybslclan_luyusheng'
		],
		ybsl_lmms: [
			'ybslshen_zhenji',
		],
		'ybsl_sgsh':[
			'sgsh_tiandi','sgsh_shennong','sgsh_xuanyuan','sgsh_shaohao','sgsh_zhuanxu',
			'sgsh_qinglong','sgsh_baihu','sgsh_zhuque','sgsh_xuanwu','sgsh_qilin',
			'sgsh_dongwanggong','sgsh_xiwangmu','sgsh_huaxu','sgsh_yaoji','sgsh_hongjunlaozu',
			'sgsh_gonggong','sgsh_zhurong','sgsh_goumang','sgsh_houtu','sgsh_yuqiang',
			'sgsh_fuxi','sgsh_xiangliu','sgsh_houyi','sgsh_dayu','sgsh_kuafu',
			'sgsh_tubo','sgsh_jingwei','sgsh_qibo','sgsh_taizichangqin','sgsh_yeming',
			'sgsh_yinglong','sgsh_nvba','sgsh_chiyou','sgsh_fenghou','sgsh_jiutianxuannv',
			'sgsh_luozu','sgsh_cangjie','sgsh_limu','sgsh_changxian','sgsh_guiyuqu',
			'sgsh_shijiamouni','sgsh_luohou','sgsh_xingtian',
		],
		ybsl_key:[
			'yb_key_hina','yb_key_kotori','yb_key_kagari',
			'ybsl_youta','ybsl_hairi','ybsl_youta','ybsl_kamome',
			'sp_key_umi',
		],
		bilibiliup: [
			'bilibiliup_jiangali',
			'bilibiliup_guanyueliuli',
			'bilibiliup_sunyuyou',
			'bilibiliup_shianshan',
			'bilibiliup_roubaozi',
			'bilibiliup_baitaosuyu',
			'bilibiliup_wanqinsin',
			'bilibiliup_xiyeqianzhi',
			'bilibiliup_lulinxikoi',
			'bilibiliup_xilingna',
			'bilibiliup_jiligulu',
			'bilibiliup_maomaobaobao',
			'bilibiliup_wenyuexu',
			'bilibiliup_xueyuer',
			'bilibiliup_qingxiaozhi',
			'bilibiliup_qinian',
			'bilibiliup_menglingli',
			'bilibiliup_xueyunshang',
			'bilibiliup_xingyao',
			'bilibiliup_zhezhi',
			'bilibiliup_xiazhisha',
			'bilibiliup_qiyuan',
			'bilibiliup_xinglisisi',
			'bilibiliup_rongxiaoxuan',
			'bilibiliup_shimizai',
			'bilibiliup_linnannanya',
			'bilibiliup_wanyue',
			'bilibiliup_taomoran',
			'bilibiliup_xiaotaosheng',
			'bilibiliup_suilin',
			'bilibiliup_baozai',
			'bilibiliup_mixieer',
			'bilibiliup_shishi',
			'bilibiliup_qingling',
			'bilibiliup_xiaoaijiang',
			'bilibiliup_xinqi',
			'bilibiliup_lushi',
			'bilibiliup_tutu',
			'bilibiliup_haitang',
			'bilibiliup_shunhua',
		],
	},
}
