...
const translate = {
	...
	zxunnamed_qinglan_info : '锁定技，每回合每项限一次，你使用牌指定其他角色为唯一目标后，其选择一项：1.弃置一张牌并视为使用一张【酒】，2.将牌堆顶的牌当做【折戟】对自己使用并回复一点体力，3.与你各摸一张牌。',
	zxunnamed_lingbo : '凌波',
	...
	ybsl_ljguihang:'归航',
	ybsl_ljguihang_info:`使命技，锁定技，当你失去手牌后，若你手牌数小于体力上限，你获得【影】补充至体力上限；成功：准备阶段，若你手牌中均为【影】，你弃置所有手牌，获得等量“橘”，并获得技能${get.poptip('nzry_huaiju')}，然后摸等量手牌（至多5张），然后增加1点体力上限并回复1点体力。`,
	// 修改后的方案：
...
// 宗族技，当你每回合首次使用一种类型的牌后，若你本回合未造成伤害，你可以令一名同族角色发动一个出牌阶段限一次的技能（不计入次数）。
	...
	*/
	...
	ybsl_yedun:'夜遁',
	ybsl_yedun_info:`锁定技，当你每回合首次受到伤害后，你需选择一名其他角色（不能是本局游戏以此法选择过的角色），其需选择：①令你摸三张牌，然后你本回合不能成为黑色牌目标；②令你失去1点体力，然后你本局游戏获得${get.poptip('')}。`,
	ybsl_yedunx:'夜遁',
	...
	hairi_zhongxia:'终夏',
	hairi_zhongxia_info:`使命技，当你需要使用一张基本牌时，你可以弃置所有手牌（至少一张），若你以此法弃置的牌花色各不相同，你视为使用之，若你以此法弃置的牌数等于你已损体力值或体力上限，你加1点体力上限（若未成功使用目标牌，此技能本回合不能再用）<br>成功：准备阶段，若你的体力上限不小于5，你选择一项：①获得技能${get.poptip('umi_chaofan')}，并恢复所有已废除装备栏。②将体力上限调整为1，并重置〖终夏〗。<br>失败：当你死亡时，你令一名其他角色获得〖终夏〗。`,
	...
	//-------------------------------周瑜小乔
	'ybsl_xianyin':'弦音',
	'ybsl_xianyin_info':`转换技，（括号内的阴阳为${get.poptip('ybsl_luanming')}的形态）<br/>阴（阴）：当你因弃置而失去一张黑桃牌时，你可令一名角色下个摸牌阶段额外摸一张牌；（若你没有〖鸾鸣〗或〖鸾鸣〗已使用则改为黑色牌）<br/>阴（阳）：当你因弃置而失去一张梅花牌时，你可令一名角色回复1点体力；（若你没有〖鸾鸣〗或〖鸾鸣〗已使用则改为黑色牌）<br/>阳（阴）：当你因弃置而失去一张红桃牌时，你可令一名角色失去1点体力；（若你没有〖鸾鸣〗或〖鸾鸣〗已使用则改为红色牌）<br/>阳（阳）：当你因弃置而失去一张方块牌时，你可令一名角色下个摸牌阶段少摸一张牌。（若你没有〖鸾鸣〗或〖鸾鸣〗已使用则改为红色牌）`,
	'ybsl_luanming':'鸾鸣',
