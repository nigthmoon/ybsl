...
game.import('character',function(lib, game, ui, get, ai, _status){ 
	...
	for(var i in ybxh.character){
		if(ybxh.character[i][4])ybxh.character[i][4].push(`ext:夜白神略/image/ybsl003/${i}.jpg`);
		else ybxh.character[i].img=`extension/夜白神略/image/ybsl003/${i}.jpg`;
	} 
	for(var i in ybxh.character){
		if(ybxh.character[i][4])ybxh.character[i][4].push('die:夜白神略/audio/die/'+i+'.mp3');
	}
