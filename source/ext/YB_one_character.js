	...
	var YB_one={ 
		...
		character:{
			...
			yb_wan_wujiangceshi:['female','qun',4,[
				...
				'yb_wan_luosuo',
				'yb_wan_qingzhu',
				'yb_wan_xueyii','yb_wan_suyi',
				'yb_wan_fushui','yb_wan_huantian','yb_wan_huangtian',
				'yb_wan_shuangqiang','yb_wan_qiangxia',
				'yb_wan_xianshuai','yb_wan_zeishi',
				'yb_wan_zhujian',
		...
		skill:{
			yb_wan_qingzhu:{//请诛换了个写法，没bug了，就是有点艹淡
				...
				content:async function(event,trigger,player){
					...
					var relu = await target1.chooseControl()
			...
			yb_wan_jiqiao:{//机巧
				...
				async content(event,trigger,player){
					...
					if(cards.length>=3){
						...
						var relu = await player.chooseControl('令他摸','令他弃','cancel2').set('ai',function(){
							var att = get.attitude(_status.event.player,target);
							if (att>0)return 0;
							return 1;
						}).set('prompt','请选择令'+get.translation(target)+'摸还是弃'+num3+'张牌').forResult();
						if(relu.index==0){
			...
			yb_wan_jianli:{//坚利
				...
				async content(event,trigger,player){
					...
					var list2=[];
					for(var k of cards){
						if(list2.length==0||!list2.includes(get.color(k)))await list2.add(get.color(k));
					}
					if(list2.length==1){
						var relu = await player.chooseTarget('选择一名角色，对其造成一点伤害').set('ai',function(target){
			...
			yb_wan_yaoji:{//邀计
				...
				content:async function(event,trigger,player){
					...
					var relu = await tarb.chooseControl()
					...
					if(relu.index==0){
						...
						if(num2>0&&tara.isAlive())await tara.draw(num2);
						if(num1>0&&tarb.isAlive())await tarb.draw(num1);
					}
					else {
			...
			yb_wan_luosuo:{//落锁
				...
				async content(event,trigger,player){
					var relu = await player.judge(function(card){//你进行一次判定
				...
				async content(event,trigger,player){
					...
					if(relu.bool){
						...
						for(var k=0;k<cards.length;k++ ){
							...
							if(!player.storage.yb_wan_qiangxia_destroy)player.storage.yb_wan_qiangxia_destroy=[];
			...
			yb_wan_yijie:{//义结
				...
				async content(event,trigger,player){
					var list=[player];
					var relu = await player.chooseTarget(get.prompt2('yb_wan_yijie'),function(card,player,target){//选1个目标
					...
					if(game.countPlayer(function(current){
						...
						var relux = await player.chooseTarget(get.prompt2('yb_wan_yijie'),function(card,player,target){//选1个目标
					...
					use:{
						...
						async content(event,trigger,player) {
							...
							if (!relu.bool) {
								...
								if(player.getCards('h').includes(cards[0]))var relu3 = await player.chooseTarget().set('ai',function(target){return false}).forResult();
				...
				async content(event,trigger,player){
					...
					while (max - list.length > 0) {
						const {
							...
							.set("list", list)
							.set("forced", max - list.length > 1).forResult();
						if (bool) {
							listx.push(cards);
				...
				subSkill:{
					...
					sha:{
						...
						async content(event,trigger,player){
							...
							var result =await player.chooseToUse(function(card,player,event){
								return get.name(card)=='sha'&&lib.filter.filterCard.apply(this,arguments);
							},source,-1).set('addCount',false)
							.set('prompt2','是否对'+get.translation(source)+'使用一张杀，令其使用的此杀无效')
							.set('logSkill','yb_wan_linzuo_sha').forResult();
							if(result.bool){
								await trigger.getParent().excluded.add(target);
								if(player.countCards('hej')>0){
									var relu = await player.choosePlayerCard(player,true,'hej').set('ai',function(button){
										let val = get.buttonValue(button);
										return -val;
									}).set('prompt2','请选择一张牌盖在武将牌上称为“勋”').forResult();;
									if(relu.bool){
								...
								if(target.countCards('hej')>0){
									var relu2 = await target.choosePlayerCard(target,true,'hej').set('ai',function(button){
										let val = get.buttonValue(button);
										return -val;
									}).set('prompt2','请选择一张牌交给'+get.translation(player)).forResult();;
									if(relu2.bool){
			...
			yb_wan_gongxun:{//功勋
				...
				async content(event,trigger,player){
					...
					if(event.triggername=='damageBegin4'){
						...
						var result = await player.chooseTarget(true).set('prompt2','令一名角色摸两张牌。').set('ai',function(tar){
						...
						var result = await player.chooseTarget(true).set('prompt2','对一名角色造成一点伤害。').set('ai',function(tar){
