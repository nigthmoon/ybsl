...
game.import('character',function(lib, game, ui, get, ai, _status){ 
	var ybdd={ 
		...
		characterSort:{
			ybdd:{
				order_xzhq:[
					'North_yanghuiyu','North_duyu','North_sunshangxiang','Northxx_sunshangxiang','North_zhugeliang','Northxx_zhugeliang','North_lidian',
					'North_diaochan','North_beimihu','North_bulianshi','North_caojinyu','North_huangyueying','North_caoxiancaohua','North_sunhanhua',
					'North_guozhao','North_zhaoxiang','North_liufeng'
				],
				order_ntr:[
					'North_shamoke'
				],
				...
				order_nobody:[
					'YB_nobody_simayi','YB_nobody_zhaoyun'
				],
			}
			
		},
		character:{ //武将格式 : 
			North_yanghuiyu:['female','shen',3,['North_yhy_xuyin','North_yhy_cihua','North_yhy_minzeng'],['jin','boss','bossallowed','legend']],
			North_duyu:['male','shen',4,['North_dy_qingyu','North_dy_zhengwu','North_dy_kuangzou'],['jin','boss','bossallowed','legend']],
			North_sunshangxiang:['female','shen',3,['North_ssx_jibing','North_ssx_lieyuan'],['wu','boss','bossallowed','legend']],
			Northxx_sunshangxiang:['female','shen',3,['North_ssx_jibing','North_ssx_lieyuanxx'],['wu','boss','bossallowed','legend']],
			North_zhugeliang:['male','shen',3,['North_zgl_zhenhu','North_zgl_dongxu','North_zgl_qizhu','North_zgl_shiyan'],['shu','boss','bossallowed','legend']],
			Northxx_zhugeliang:['male','shen',3,['North_zgl_zhenhu','North_zgl_dongxu','North_zgl_qizhu','North_zgl_shiyanxx'],['shu','boss','bossallowed','legend']],
			North_lidian:['male','shen',3,['North_ld_chenxun','North_ld_minde'],['wei','boss','bossallowed','legend']],
			North_shamoke:['male','shen',4,['North_smk_shangying'],['shu','boss','bossallowed','legend']],
			North_diaochan:['female','shen',3,['North_dc_ruofu','North_dc_sulian','North_dc_qilu','North_dc_ziman'],['qun','boss','bossallowed','legend']],
			North_beimihu:['female','shen',3,['North_bmh_wuzhi','North_bmh_huanchao','North_bmh_chizhangt','North_bmh_lushou'],['qun','boss','bossallowed','legend']],
			North_bulianshi:['female','shen',3,['North_bls_qiangong','North_bls_yuanya'],['wu','boss','bossallowed','legend']],
			North_caojinyu:['female','shen',3,['North_cjy_bashu','North_cjy_duijing','North_cjy_lvzhi'],['wei','boss','bossallowed','legend']],
			North_huangyueying:['female','shen',3,['North_hyy_guishi','North_hyy_lancai','North_hyy_bingxue'],['qun','boss','bossallowed','legend']],
			...
			North_caoxiancaohua:['female','shen',3,['North_cxch_lingxi','North_cxch_gongsheng','North_cxch_lianyu','North_cxch_xixuan'],['qun','boss','bossallowed','legend']],
			North_sunhanhua:['female','shen',3,['North_shh_yuniao','North_shh_qingsi','North_shh_xianyin'],['wu','boss','bossallowed','legend']],
			...
			"YB_nobody_simayi":["male","wei",3,["YB_nobody_guiyin","YB_nobody_linglv"],["ext:夜白神略/image/ybdd/YB_nobody_simayi.gif"]],
			"YB_nobody_zhaoyun":["male","qun",4,["YB_nobody_longying","YB_nobody_fengguo"],["ext:夜白神略/image/ybdd/YB_nobody_zhaoyun.gif"]],
		...
		skill:{
			...
			North_yhy_cihua:{
				...
				subSkill:{
					...
					if(event.player!=player&&event.card.isCard&&event.player.isPhaseUsing()){
						return (event.player.getHistory('useCard').indexOf(event)==player.hp-1&&['basic','trick'].contains(get.type(event.card)));
					}
				},
			...
			North_dy_qingyu:{
					...
					content:function(storage,player,skill){
						...
						for(var j=0;j<list.length;j++){
							...
							if(list2.contains(list[j])){
								if(list3.contains(list[j])){str+='<span style="text-decoration: line-through;"><span class=yellowtext>'+get.translation(list[j])+'</span></span>'}
				...
				upc:function(){
					...
					player.chooseButton(dialog).set('ai',function(button){
						...
						var player=_status.event.player;
						if(player.storage.North_dy_qingyu_light.contains(button.link[2]))return false;
						return true;
					});
					'step 1'
					if(result.bool){
				...
				subSkill:{
					...
					use2:{
						...
						filter:function(event,player){
							...
							var list=player.storage.North_dy_qingyu_light;
							if(event.name.indexOf('lose')!=0) return (list.contains(event.card.name)&&get.type(event.card)=='basic'&&event.card.isCard);
						},
							...
							dialog:function (event,player){
								...
								for(var i of player.storage.North_dy_qingyu_light){
									...
									if(!player.storage.North_dy_zhengwu_length){
										...
										if(!player.storage.North_dy_zhengwu_length.contains(i))list.push(['整武','',i]);
										if(i=='sha'){
											for(var j of lib.inpile_nature){
												card.nature=j;
												if(!player.storage.North_dy_zhengwu_length.contains(i)) list.push(['整武','',i,j]);
											}
										}
									}
								}
			...
			North_zgl_shiyan:{
				...
				'prompt2':function(event,player){
					...
					var suit=[];
					for(var i of cards){
						if(!suit.contains(get.suit(i)))suit.push(get.suit(i));
					}
			...
			North_dc_ziman2:{
				...
				filter:(event,player)=>{
					return player!=event.player&&!event.player._trueMe&&player.storage.North_dc_ziman.contains(event.player);
				},
				logTarget:'player',
				content:function(){
			...
			North_bls_qiangong:{
				...
				filterCard:function(card,player){
					...
					return !lib.skill.North_bls_qiangong.getsuit(player).contains(get.suit(card));
				},
				selectTarget:2,
				filterTarget:function(card,player,target){
					if(ui.selected.targets.length==1){
						return !lib.skill.North_bls_qiangong.gettar2(player).contains(target);
					}
					return !lib.skill.North_bls_qiangong.gettar1(player).contains(target);
				},
				// multiline:true,
				multitarget:true,
				mark:true,
				...
				content:function(){
					...
					if(result.targets){
						...
						for(var i=0;i<list.length;i++){
							if(player.storage['North_bls_yuanya_'+result.targets[0].playerid].contains(list[i])){
						...
						filter:function(event,player){
							...
							return tar.getHistory('useCard',function(evt){
								if(evt.targets&&evt.targets.length&&evt.isPhaseUsing()){
									...
									// while(targets.contains(tar)) targets.remove(tar);//筛除自己
			...
			North_cxch_gongsheng:{
				...
				filter:function(event,player){
					if(event.name=='addToExpansion'){
						if(event.gaintag.contains('North_cxch_lingxi_yuping')){
						...
						else if(event.gaintag.contains('North_cxch_lingxi_tingniao')){
					...
					if(event.name=='lose'&&event.getlx!==false){
						for(var i in event.gaintag_map){
							if(event.gaintag_map[i].contains('North_cxch_lingxi_yuping')){
								var cards2=event.player.getExpansions('North_cxch_lingxi_yuping').filter((item)=>{for(var k of event.cards){return item!=k}});
								var cards=event.player.getExpansions('North_cxch_lingxi_yuping');
								var cards3=event.player.getExpansions('North_cxch_lingxi_yuping').concat(event.cards);
								if(!cards.length||get.YB_suit(cards3,'type2').length!=get.YB_suit(cards,'type2').length||!cards2.length||get.YB_suit(cards2,'type2').length!=get.YB_suit(cards,'type2').length) {
							...
							else if(event.gaintag_map[i].contains('North_cxch_lingxi_tingniao')){
								var cards2=event.player.getExpansions('North_cxch_lingxi_tingniao').filter((item)=>{for(var k of event.cards){return item!=k}});
								var cards=event.player.getExpansions('North_cxch_lingxi_tingniao');
								var cards3=event.player.getExpansions('North_cxch_lingxi_tingniao').concat(event.cards);
								if(!cards.length||get.YB_suit(cards3).length!=get.YB_suit(cards).length||!cards2.length||get.YB_suit(cards2).length!=get.YB_suit(cards).length) {
					...
					return game.getGlobalHistory('cardMove',function(evt){
						...
						for(var i in evt.gaintag_map){
							if(evt.gaintag_map[i].contains('North_cxch_lingxi_yuping')){
								var cards2=event.player.getExpansions('North_cxch_lingxi_yuping').filter((item)=>{for(var k of event.cards){return item!=k}});
								var cards=event.player.getExpansions('North_cxch_lingxi_yuping');
								var cards3=event.player.getExpansions('North_cxch_lingxi_yuping').concat(event.cards);
								if(!cards.length||get.YB_suit(cards3,'type2').length!=get.YB_suit(cards,'type2').length||!cards2.length||get.YB_suit(cards2,'type2').length!=get.YB_suit(cards,'type2').length) {
							...
							else if(evt.gaintag_map[i].contains('North_cxch_lingxi_tingniao')){
								var cards2=event.player.getExpansions('North_cxch_lingxi_tingniao').filter((item)=>{for(var k of event.cards){return item!=k}});
								var cards=event.player.getExpansions('North_cxch_lingxi_tingniao');
								var cards3=event.player.getExpansions('North_cxch_lingxi_tingniao').concat(event.cards);
								if(!cards.length||get.YB_suit(cards3).length!=get.YB_suit(cards).length||!cards2.length||get.YB_suit(cards2).length!=get.YB_suit(cards).length) {
			...
			"North_PC_qianshui":{
				...
				filter:function(event,player,name){
					if(name=='roundStart'){
						...
						if(event.name=='lose'){
							for(var i in event.gaintag_map){
								if(event.gaintag_map[i].contains('North_PC_qianshui_tag')) return true;
							}
							return false;
						}
						return player.hasHistory('lose',function(evt){
							...
							for(var i in evt.gaintag_map){
								if(evt.gaintag_map[i].contains('North_PC_qianshui_tag')) return true;
							}
							return false;
						});
					}
				...
				content:function(){
					...
					if(trigger.name=='lose'){
						for(var i in trigger.gaintag_map){
							if(trigger.gaintag_map[i].contains('North_PC_qianshui_tag')) num++;
						}
					}
					else {
						player.getHistory('lose',function(evt){
							...
							for(var i in evt.gaintag_map){
								if(evt.gaintag_map[i].contains('North_PC_qianshui_tag')) num++;
							}
						});
					}
					event.num=num;
			...
			"North_mouhua":{
				...
				hiddenCard:function (player,name){
					...
					return list.contains(name)&&player.countCards('hes')>=1&&!player.hasSkill('North_mouhua_block');
				},
			...
			"YB_nobody_guiyin":{
				...
				filter:function(event,player,name){
					...
					if(players.length!=1)return name=='useCard'&&['basic','trick'].contains(get.type(event.card));
			...
			"YB_nobody_fengguo":{
				...
				content:function(){
					var target=trigger.player;
					'step 0'
					var list=['与'+player+'各摸两张牌','本局游戏对因“龙缨”受到过伤害的角色造成的伤害＋1'];
	...
	if(lib.device||lib.node){ 
		for(var i in ybdd.character){ybdd.character[i][4].push('ext:夜白神略/image/ybdd/'+i+'.jpg');} 
	}else{ 
		for(var i in ybdd.character){ybdd.character[i][4].push('db:extension-夜白神略:'+i+'.jpg');} 
	}//由于以此法加入的武将包武将图片是用源文件的，所以要用此法改变路径 
	for(var i in ybdd.character){
		ybdd.character[i][4].push('die:夜白神略/audio/die/'+i+'.mp3');
	}
	return ybdd; 
}); 
