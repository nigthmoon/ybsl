...
game.import('character',function(lib, game, ui, get, ai, _status){ 
	var ybart={ 
		...
		character:{ //武将格式 : 
			'ybart_013yinji':['female','YB_memory',3,['yb017_shanwu','dz013_qingling'],['epic']],//尹姬
			'ybart_014liutianyu':['male','YB_memory',4,['yb014_shifu','yb014_huanlei'],['legend']],//夜白
			'ybart_016manchengqi':['female','YB_memory',4,['dz016_zanxu','yb017_shanwu'],['epic']],//满城柒
			'ybart_017xiaohong':['female','YB_memory',3,['yb017_luming','yb017_zhengxiong','yb017_shanwu'],['legend']],//小红
			'ybart_041mmuqin':['female','YB_memory',3,['yb041_juxin','yb041_sange'],['epic']],//慕琴
		},
		skill:{
			//---------------六艺篇
			'_ybsl_sixart':{
				...
				filter:function(event,player){
					return player.getHistory('lose',function(evt){
						...
						for(var i in evt.gaintag_map){
							if(evt.gaintag_map[i].contains('_ybsl_sixart')) return true;
						}
						return false;
					}).length>0;
			...
			'yb017_zhengxiong':{
				...
				filter:function(event,player){
					return (!player.storage.yb017_zhengxiong||!player.storage.yb017_zhengxiong.contains(event.player))&&event.player.hasSex('male');
				},
				content:function(){
					'step 0'
					player.draw(2);
			...
			yb014_shifu:{
				...
				hiddenCard:function(player,name){
					return (!player.getStorage('yb014_shifu_block').contains(name)&&player.countCards('s',function(card){
						return card.hasGaintag('_ybsl_sixart')
					})>0&&lib.inpile.contains(name));
				},
				init:function(player){
				...
				chooseButton:{
					...
					var list=[];
						for(var i=0;i<lib.inpile.length;i++){
							...
							if(player.storage.yb014_shifu_block&&player.storage.yb014_shifu_block.contains(name)) continue;
			...
			yb014_shifu5:{
				...
				viewAsFilter:function(player){
					return !player.getStorage('yb014_shifu_block').contains('wuxie')&&player.countCards('s',function(card){
						return card.hasGaintag('_ybsl_sixart')
					});
				},
				onuse:function(result,player){
				...
				intro:{
					...
					content:function(storage,player,skill){
						...
						for(var i in list){
							var j='ybsl_'+i;
							var k='ybmyx_'+i;
							if(player.storage[j].contains('key')){
								str+='<br>主流派<br>',
							...
							else if(player.storage[j].contains('sec')){
								str+='<br>支流派<br>'
								str+=player.storage[j+'_lv']+'---'+player.storage.yb014_shifu2+'/'+(player.storage[j+'_lv']+1)*game.checkMod(event,player,0,'ybsl'+i+'Mod',player)+'<br>';
								for (var p=0;p<5;p++){
			...
			'ybsl_moon':{
				...
				mod:{
					ybslmoonMod:function(event,player,num){
						var str=Infinity;
						if(player.storage.ybsl_moon.contains('key'))str=50;
						if(player.storage.ybsl_moon.contains('sec'))str=35;
						return num+str;
					},
					ybslmoonlvMod:function(event,player,num){
						var str=0;
						if(player.storage.ybsl_moon.contains('key'))str=8;
						else if(player.storage.ybsl_moon.contains('sec'))str=5;
						return num+str;
					},
				},
				up:function(player){
			...
			'ybsl_clam':{
				...
				mod:{
					ybslclamMod:function(event,player,num){
						var str=Infinity;
						if(player.storage.ybsl_clam.contains('key'))str=40;
						if(player.storage.ybsl_clam.contains('sec'))str=28;
						return num+str;
					},
					ybslclamlvMod:function(event,player,num){
						var str=0;
						if(player.storage.ybsl_clam.contains('key'))str=8;
						else if(player.storage.ybsl_clam.contains('sec'))str=5;
						return num+str;
					},
				},
				up:function(player){
			...
			'ybsl_lightning':{
				...
				mod:{
					ybsllightningMod:function(event,player,num){
						var str=Infinity;
						if(player.storage.ybsl_lightning.contains('key'))str=40;
						if(player.storage.ybsl_lightning.contains('sec'))str=28;
						return num+str;
					},
					ybsllightninglvMod:function(event,player,num){
						var str=0;
						if(player.storage.ybsl_lightning.contains('key'))str=8;
						else if(player.storage.ybsl_lightning.contains('sec'))str=5;
						return num+str;
					},
				},
				up:function(player){
			...
			'ybsl_wind':{
				...
				mod:{
					ybslwindMod:function(event,player,num){
						var str=Infinity;
						if(player.storage.ybsl_wind.contains('key'))str=50;
						if(player.storage.ybsl_wind.contains('sec'))str=35;
						return num+str;
					},
					ybslwindlvMod:function(event,player,num){
						var str=0;
						if(player.storage.ybsl_wind.contains('key'))str=8;
						else if(player.storage.ybsl_wind.contains('sec'))str=5;
						return num+str;
					},
				},
				up:function(player){
			...
			'ybsl_flame':{
				...
				mod:{
					ybslflameMod:function(event,player,num){
						var str=Infinity;
						if(player.storage.ybsl_flame.contains('key'))str=20;
						if(player.storage.ybsl_flame.contains('sec'))str=14;
						return num+str;
					},
					ybslflamelvMod:function(event,player,num){
						var str=0;
						if(player.storage.ybsl_flame.contains('key'))str=8;
						else if(player.storage.ybsl_flame.contains('sec'))str=5;
						return num+str;
					},
				},
				up:function(player){
	...
	if(lib.device||lib.node){ 
		for(var i in ybart.character){ybart.character[i][4].push('ext:夜白神略/image/character/'+i+'.jpg');} 
	}else{ 
		for(var i in ybart.character){ybart.character[i][4].push('db:extension-夜白神略:'+i+'.jpg');} 
	}//由于以此法加入的武将包武将图片是用源文件的，所以要用此法改变路径 
	for(var i in ybart.character){
		ybart.character[i][4].push('die:夜白神略/audio/die/'+i+'.mp3');
	} 
	for(var i in ybart.card){
		if(!ybart.card[i].image&&!ybart.card[i].modeimage) ybart.card[i].image='ext:夜白神略/image/card/'+i+'.png'
	}
	return ybart; 
});